...
func zlmGetMediaList(req zlmGetMediaListReq) zlmGetMediaListResp {
	...
	body, err := sip.GetRequest(config.Media.RESTFUL + reqStr)
	if err != nil {
		// logrus.Errorln("get stream mediaList fail,", err)
		return res
	}
	if err = sip.JSONDecode(body, &res); err != nil {
		// logrus.Errorln("get stream mediaList fail,", err)
		return res
	}
...
func zlmGetMediaInfo(ssrc string) rtpInfo {
	res := rtpInfo{}
	body, err := sip.GetRequest(config.Media.RESTFUL + "/index/api/getRtpInfo?secret=" + config.Media.Secret + "&stream_id=" + ssrc)
	if err != nil {
...
func zlmCloseStream(ssrc string) {
	sip.GetRequest(config.Media.RESTFUL + "/index/api/close_streams?secret=" + config.Media.Secret + "&stream=" + ssrc)
}

// zlm 开始录制视频流
func zlmStartRecord(values url.Values) error {
	body, err := sip.GetRequest(config.Media.RESTFUL + "/index/api/startRecord?" + values.Encode())
	if err != nil {
		return err
	}
...
func zlmStopRecord(values url.Values) error {
	body, err := sip.GetRequest(config.Media.RESTFUL + "/index/api/stopRecord?" + values.Encode())
	if err != nil {
		return err
	}
	tmp := map[string]interface{}{}
